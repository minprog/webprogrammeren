...
Download [movies.zip](https://github.com/minprog/webprogrammeren/raw/2020/Problems/0%20Movies/movies.zip) and unzip its contents. Move to an appropriate directory.

## Understanding

Provided to you is a file called `movies.db`, a SQLite database that stores data from [IMDb](https://www.imdb.com/) about movies, the people who directed and starred in them, and their ratings. In a terminal window, then:
...
The tables in `movies.db` have been created as follows:
...
By examining those statements, you can identify the columns present in each table. Notice that the `movies` table has an `id` column that uniquely identifies each movie, as well as columns for the `title` of a movie and the `year` in which the movie was released. The `people` table also has an `id` column, and also has columns for each person's `name` and `birth` year.
...
You're welcome to check your queries' results against [IMDb](https://www.imdb.com/) itself, but realize that ratings on the website might differ from those in `movies.db`, as more votes will have been cast since we downloaded the data!

You can develop your SQL queries in Python using `execute()` and `fetchone()` or `fetchall()` but eventually you will need to create small SQL files that only contain the SQL query. We will use these files to check your answers automatically.
...
* To see how many rows are returned by your query, you can save the result of `fetchall()` in a variable and then use Python's built-in `len()` function to get the number of rows.
