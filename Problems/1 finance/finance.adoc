...
This problem will be graded on code quality, primarily on the following aspects:

- link:/quality/comments[Comments]
- link:/quality/naming[Naming]
- link:/quality/duplication[Duplication]
- link:/quality/nesting[Nesting]
...
You're about to implement C$50 Finance, a web app via which you can manage portfolios of stocks. Not only will this tool allow you to check real stocks' actual prices and portfolios' values, it will also let you buy (okay, "buy") and sell (okay, "sell") stocks by querying iextrading.com for stocks' prices.
...
[source,subs="macros"]
----
$ wget https://webik.mprog.nl/course/problems/1%20finance/finance.zip
$ unzip finance.zip
$ rm finance.zip
$ cd finance
$ ls
...
[source]
----
check50 minprog/cs50x/2019/finance
----
...
*In your `finance` folder (the one that contains `application.py`), run the following command to create your zip file:*

[source]
----
zip -r -9 finance.zip application.py finance.db helpers.py requirements.txt static templates
----

Download that zipfile and make sure it is not unzipped on your own computer!

Don't forget to check on this same website that our `check50` agrees that your submission is complete and correct. It will take a couple of minutes to test.
